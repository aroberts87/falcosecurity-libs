...
TEST_F(sinsp_with_test_input, THRD_TABLE_traverse_default_tree) {
	...
	sinsp_threadinfo* tinfo = thread_manager->find_thread(p4_t1_tid, true).get();
	ASSERT_TRUE(tinfo);
	...
	tinfo = thread_manager->find_thread(p5_t2_tid, true).get();
	ASSERT_TRUE(tinfo);
	...
	tinfo = thread_manager->find_thread(p6_t1_tid, true).get();
	ASSERT_TRUE(tinfo);
...
TEST_F(sinsp_with_test_input, THRD_TABLE_args_env_subtable_resolves_to_leader) {
	...
	auto leader = m_inspector.m_thread_manager->find_thread(leader_tid, true);
	auto secondary = m_inspector.m_thread_manager->find_thread(secondary_tid, true);
	...
	auto read_subtable = [&](const char* field_name, int64_t tid) -> std::vector<std::string> {
		auto field = table->static_fields()->find(field_name);
		EXPECT_NE(field, table->static_fields()->end());
		auto acc = field->second.into<libsinsp::state::base_table*>();
		...
		const auto& vfield = subtable->get_field("value", SS_PLUGIN_ST_STRING);
		auto vacc = vfield.as<std::string>();
