...
TEST_F(sys_call_test, unix_client_server) {
	...
	event_filter_t filter = [&](sinsp_evt* evt) {
		...
		if(ti) {
			const auto& args = ti->get_args();
			if(ti->get_comm() == "python3" && args.size() >= 1) {
				return ends_with(args[0], "unix_client_server.py") ||
				       ends_with(args[0], "unix_client_server.py");
			}
		}
		return false;
	};
	...
	captured_event_callback_t callback = [&](const callback_param& param) {
		...
		if(evt->get_type() == PPME_SOCKET_SEND_X) {
			...
			callnum++;
		} else if(evt->get_type() == PPME_SOCKET_SENDTO_X) {
			callnum++;
